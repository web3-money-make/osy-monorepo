...
async function main() {
  ...
  // 5. set bridge
  await bridgeInstance.setUSDC(usdc[chainId]);
  ...
  await omniVaultInstance.setUSDC(usdc[chainId]);
  ...
  await omniVaultInstance.setComet(compound[chainId]);
  console.log("Compound set in OmniVault");
